...
export const deleteMyAccount = async (confirmEmail: string) => {
  ...
  if (fnError) {
    // 兜底：Edge Function 未部署时，回退到 RPC 仅做“计划删除”标记
    const { error } = await (supabase as any).rpc('request_account_deletion', { confirm_email: email })
    if (error) throw error
  }
...
export const updateAllowShare = async (userId: string, value: boolean) => {
  const { error } = await supabase
    .from('profiles')
    .update({ allow_share: value } as any)
    .eq('id', userId);
  if (error) console.warn('同步 allow_share 失败（已忽略）:', error.message);
};
