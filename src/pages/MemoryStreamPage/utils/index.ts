...
export const META_PREFIX = '[orbit_meta:';

export const encodeSharePayload = (payload: Record<string, any>) => {
...
export const decodeMemoryContent = (content: string): { text: string; weather: string; mood: string; route: string } => {
...
export const formatDateGroup = (dateStr: string) => {
  ...
  if (year === today.getFullYear()) return `${month}月${day}日 ${weekday}`;
