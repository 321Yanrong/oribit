...
export const MemoryStoryDrawer = ({
  ...
  return (
    <AnimatePresence>
                  ...
                  {currentItem.isAllLast && storyCompleted && (
                    ...
                    >
                      ...
                      <div className="flex gap-3 flex-col sm:flex-row">
                        <button
                          onClick={onClose}
                          className={`flex-1 py-3.5 rounded-2xl font-semibold text-sm border ${theme === 'light'
                        ...
                        <button
                          ...
                          className={`flex-[1.6] py-3.5 rounded-2xl bg-gradient-to-r from-[#00FFB3] to-[#00D9FF] font-bold text-sm shadow-[0_0_20px_rgba(0,255,179,0.3)] transition-transform ${posterLoading ? 'opacity-80 cursor-wait' : 'hover:scale-[1.02]'
                            }`}
      ...
      <div
        className="fixed right-4 z-[220] flex items-center justify-end pointer-events-auto"
        style={{
          // 距离底部：安全区高度 + 24px (你可以根据需要调大或调小这个 24px)
          bottom: 'calc(env(safe-area-inset-bottom, 0px) + 24px)'
        }}
      >
        {/* 减小了外层容器的 px 和 py */}
        <div className="flex items-center gap-1.5 bg-white/80 text-[#0f172a] dark:bg-black/60 dark:text-white backdrop-blur-xl px-2 py-1 rounded-full border border-white/80 dark:border-white/12 shadow-lg">
          <button
            onClick={handleToggleMusic}
            className={`w-7 h-7 rounded-full border flex items-center justify-center transition-all ${isMusicPlaying
          ...
          >
            {isMusicPlaying ? <FaPause className="text-[10px]" /> : <FaPlay className="text-[10px] ml-0.5" />}
          </button>
          <button
            onClick={handleNextTrack}
            className="flex items-center gap-1.5 pl-1 pr-1.5 py-0.5 rounded-full hover:bg-white/60 dark:hover:bg-white/5 transition-colors"
            title="下一首"
          >
